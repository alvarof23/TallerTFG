		...
		$.ajax({
			...
			success: function (response) {
				if (response.status === "success") {
					...
					tabla.rows.add($(response.response)).draw();
				} else {
	...
	<?php if ($reference == "Detalles"): ?>
		...
		$(document).ready(function () {
			...
			$(document).on("click", "#pedirCita", function (event) {
				...
				var hora = $(this).data("textoBoton") || ""; // recuperar el texto guardado
				var id_vehiculo = document.getElementById("info_id_vehiculo").value;
				...
				var fecha = $("#fecha").val();
				var id_tarea = $("select[name='areasTrabajo']").val();
				...
				$.ajax({
					...
					type: "POST",
					data: { id_tarea: id_tarea, hora: hora, fecha: fecha, id_vehiculo: id_vehiculo, id_cita: id_cita, estado: estado },
					dataType: "json",
					success: function (data) {
						alert("Vehículo actualizado con exito");
			...
			$(document).on("click", ".fila-cita", function () {
				...
				let url = "<?= base_url('taller_citas/detalles') ?>/" + id_cita + "/" + id_cliente;
		...
		document.addEventListener('DOMContentLoaded', function () {
			...
			$(document).on("click", ".activar_modal_recogida", function () {
				...
				$.ajax({
					url: "<?= base_url('citas/get_recepcion') ?>/" + cita_id,
					type: 'GET',
					dataType: 'json',
					success: function (response) {
						if (response.recepcion_existente) {
							// Mostrar modal de confirmación si ya hay recepción
							$('#modalConfirmacionRecepcion').modal('show');


							$('#editRecepcion').off('click').on('click', function (e) {
								$('#modalConfirmacionRecepcion').modal('hide');
								...
								$('img.preview-impacto').remove();

								$('#modal_recogida .preview-container').empty();
								...
								$("#combustible").val(response.combustible).selectpicker('refresh').trigger("change");
								$('#observaciones_cliente').val(response.observaciones);
								$('#confirmacion_cliente').prop('checked', response.confirmacion_cliente == 1);

								$('#modal_recogida').on('shown.bs.modal', function () {
									cargarFirma();
								});
											...
											setTimeout(function () {
												...
												if (inputFile.length && previewContainer.length) {
													...
													img.addEventListener('click', function () {
														...
														$(document).off("click", "#ampliarImagen").on("click", "#ampliarImagen", function () {
															$('#modalConfirmacionImagen').modal('hide');
															$('#imageModal').modal('show');
															document.getElementById("modalImage").src = image + imagen;
														});

														$(document).off("click", "#cambiarImagen").on("click", "#cambiarImagen", function () {
															$('#modalConfirmacionImagen').modal('hide');
															inputFile.click();
														});
													});
							...
							$('#delRecepcion').off('click').on('click', function (e) {
								e.preventDefault();

								const citaId = $('#cita_id').val(); // o usa el ID que guardaste antes
								const recepcionId = response.id; // o usa el ID que guardaste antes
								$.ajax({
									url: "<?= base_url('taller_citas/del_recepcion/') ?>/" + recepcionId + "/" + citaId,
									type: 'GET',
									dataType: 'json',
									success: function (response) {
										if (response.status === 'success') {
											console.log("Se eliminó la recepción");
											$('#modalConfirmacionRecepcion').modal('hide');
											console.log("HEcho");
											location.reload();
										} else {
									...
									error: function (xhr) {
										console.error("Error al eliminar recepción:", xhr.responseText);
										alert("Error inesperado al eliminar la recepción.");
									}
								});
							});
							...
							// No hay recepción previa, abrir modal directamente
							$('#modal_recogida').modal('show');
					...
					error: function (xhr, status, error) {
						console.error("Error al obtener datos de recepción:", xhr.responseText);
						// En caso de error, abre igualmente el modal vacío
						$('#modal_recogida').modal('show');
					}
				});
			});
			...
			function cargarFirma() {
				...
				if (!canvas) {
					console.warn("Canvas firma no encontrado.");
					return;
				}
				...
				$.ajax({
					url: "<?= base_url('citas/cargar_firma') ?>/" + citaId,
					method: 'GET',
					dataType: 'json',
					success: function (response) {
						if (response.existe && response.firmaBase64) {
							const img = new Image();
							img.src = response.firmaBase64; // Usamos directamente la cadena base64
							...
							console.log("No hay firma para esta cita:", response.error);
						}
					},
					error: function (xhr, status, error) {
			...
			canvas.addEventListener("mousedown", (e) => {
				pintando = true;
				// Inicia una nueva línea en el momento del clic
				lastX = e.clientX - canvas.getBoundingClientRect().left;
				lastY = e.clientY - canvas.getBoundingClientRect().top;
				ctx.beginPath(); // Comienza una nueva línea
				ctx.moveTo(lastX, lastY); // Establece el punto de inicio
			});
			...
			canvas.addEventListener("mousemove", (e) => {
				...
				// Dibuja una línea desde el último punto a la nueva posición
				ctx.lineWidth = 2;
				ctx.lineCap = "round";
				ctx.strokeStyle = "#000";
			...
			$('#pedirRecepcion').on('click', function (e) {
				...
				let canvas = document.getElementById('firmaCanvas'); // o el ID real de tu canvas
				console.log("canvas: " + canvas);
				if (canvas) {
					formData.append('firma_base64', $('#firmaBase64').val()); // Solo si hay firma
				}
				...
				$.ajax({
					url: "<?= base_url('taller_citas/guardar_o_actualizar') ?>",
					type: 'POST',
					data: formData,
					dataType: 'json',
					processData: false,   // 🔧 evita que jQuery intente convertir formData a string
					contentType: false,   // 🔧 deja que el navegador configure el Content-Type correctamente
					success: function (response) {
						if (response.status === "success") {
			...
			$(document).on("click", ".activar_factura", function (event) {
				event.preventDefault();

				const citaId = $(this).data('id'); // ID de la cita

				$.ajax({
					url: '<?= base_url("taller_citas/getDatosPresupuesto") ?>',
					method: 'POST',
					data: { cita_id: citaId },
					dataType: 'json',
					success: function (res) {
						if (res.existe_presupuesto) {
							// Mostrar solo el botón Editar y ocultar Crear
							$('#editarPresupuesto')
								.show()
								.attr('data-id_presupuesto', res.presupuesto.id)
								.attr('data-id', res.presupuesto.cita_id)
								.attr('data-id_tarea', res.presupuesto.tarea_id);
							...
							$('#crearPresupuesto').hide();
						} else {
							// Mostrar solo el botón Crear y ocultar Editar
							$('#crearPresupuesto')
								.show()
								.data('id', res.presupuesto.id)
								.data('id_tarea', res.presupuesto.id_tarea);

							$('#editarPresupuesto').hide();
							$('#delPresupuesto').hide()
						}
					...
					error: function () {
						alert('No se pudo comprobar el estado del presupuesto.');
					}
				});
			});
			...
			$(document).on("click", "#ver_pdf", function () {
				...
				var clienteId = $(this).data("id_cliente");
				var vehiculoId = $(this).data("id_vehiculo");
				var estado = $(this).data("estado");
				var observaciones = $(this).data("observaciones");
				var fecha = $(this).data("fecha");
				var hora = $(this).data("hora");
				var tareaId = $(this).data("id_tarea");



				window.open("<?= base_url('taller_citas/imprimir_factura') ?>/" + citaId, "_blank");
			});
			...
			$(document).on("click", "#crearPresupuesto", function () {
				...
				$("#formSeleccionProductos").data("id_cita", citaId);
				$("#formSeleccionProductos").data("id_tarea", tareaId);

				$.ajax({
					url: "<?= base_url('taller_citas/getProductos') ?>",
					type: "POST",
						...
						if (data.status === 'ok') {
							...
							if (productos.length > 0) {
								...
								<tr>
									...
									<td><input type="number" name="cantidades[]" class="form-control cantidad-input" min="1" value="1" data-producto-id="${producto.id}" ${producto.seleccionado ? '' : 'disabled'}></td>
							...
							`;
								...
								html = `<tr><td colspan="4" class="text-center">No hay productos disponibles.</td></tr>`;
							}

							$("#tabla-productos-body").html(html);
						} else {
							$("#tabla-productos-body").html(`<tr><td colspan="4" class="text-danger text-center">${data.message}</td></tr>`);
						}
					},
					error: function () {
						$("#tabla-productos-body").html(`<tr><td colspan="4" class="text-danger text-center">Error al cargar los productos.</td></tr>`);
					}
				});
			...
			$(document).on('change', 'input[name="productos[]"]', function () {
				const isChecked = $(this).is(':checked');
				const productoId = $(this).val();
				$(`input.cantidad-input[data-producto-id="${productoId}"]`).prop('disabled', !isChecked);
			});

			$(document).on("click", "#guardarPresupuesto", function () {
				var id_cita = $("#formSeleccionProductos").data("id_cita");
				var id_tarea = $("#formSeleccionProductos").data("id_tarea");
				var productosSeleccionados = [];

				$("#formSeleccionProductos input[name='productos[]']:checked").each(function () {
					var productoId = $(this).val();
					var cantidad = $(`input.cantidad-input[data-producto-id="${productoId}"]`).val();
				...
				// Enviar con AJAX
				$.post("<?= base_url('taller_citas/guardar_presupuesto') ?>", {
					cita_id: id_cita,
					tarea_id: id_tarea,
			...
			$(document).on("click", "#delPresupuesto", function () {
				const presupuestoId = $(this).data("id_presupuesto");
				...
				$.ajax({
					url: "<?= base_url('taller_citas/eliminar_presupuesto') ?>",
					type: "POST",
					data: { id_presupuesto: presupuestoId },
					dataType: "json",
					success: function (res) {
						if (res.status === "ok") {
							alert("Presupuesto eliminado correctamente.");
							$('#modalPresupuesto').modal('hide');
							// Opcional: recargar la página o tabla de citas
							// location.reload();
						} else {
			...
			$(document).on("click", "#editarPresupuesto", function () {
				$('#modalPresupuesto').modal('hide'); // Oculta el modal principal
				$('#modalSeleccionProductos').modal('show'); // Muestra el modal de productos

				const citaId = $(this).data("id");
				const tareaId = $(this).data("id_tarea");
				const presupuestoId = $(this).data("id_presupuesto");

				$("#formSeleccionProductos").data("id_cita", citaId);
				$("#formSeleccionProductos").data("id_tarea", tareaId);
				$("#formSeleccionProductos").data("id_presupuesto", presupuestoId);

				// Petición AJAX para traer productos con su estado de selección
				$.ajax({
					url: "<?= base_url('taller_citas/get_presupuesto_detalle') ?>",
					type: "POST",
					data: {
					...
					success: function (res) {
						if (res.success) {
							...
							if (res.productos.length > 0) {
								res.productos.forEach(function (producto) {
									html += `
										<tr>
											<td>
												<input type="checkbox" name="productos[]" value="${producto.id}" ${producto.seleccionado ? 'checked' : ''}>
											</td>
											<td>
												<input type="number" name="cantidades[]" class="form-control cantidad-input" min="1"
													value="${producto.cantidad}" data-producto-id="${producto.id}" ${producto.seleccionado ? '' : 'disabled'}>
											</td>
											<td>${producto.nombre}</td>
											<td>${producto.descripcion_p}</td>
											<td>${parseFloat(producto.precio).toFixed(2)}</td>
							...
							$("#tabla-productos-body").html(`<tr><td colspan="4" class="text-danger text-center">${res.message}</td></tr>`);
						}
					},

					error: function () {
						$("#tabla-productos-body").html(`<tr><td colspan="4" class="text-danger text-center">Error al cargar los productos del presupuesto.</td></tr>`);
					}
				});
			});
...
<?php if ($this->input->get('nueva') == 1): ?>
    $(document).ready(function() {
        $('.activar_modal_add').trigger('click');

    });
<?php elseif ($this->input->get('estado') == 'Esperando_recepcion'): ?>
	$(document).ready(function() {
		$.ajax({
			url: "<?= base_url('taller_citas/recargar_tabla_citas') ?>",
			type: "POST",
			data: {
				botonPresionado: "redirigido"
			},
			dataType: "json",
			success: function (response) {
				if (response.status === "success") {
