...
<script>
	<?php if ($reference == "Detalles"): ?>
		...
		document.addEventListener('DOMContentLoaded', function () {
				...
				$.ajax({
					...
					success: function (response) {
						if (response.recepcion_existente) {
							...
							// Click en "Editar" dentro del modal de confirmación
							...
							$('#editRecepcion').off('click').on('click', function (e) {
								...
								// ✅ Vaciar previews correctamente
								console.log('Antes de vaciar:', $('#modal_recogida .preview-container img').length);
								$('#modal_recogida .preview-container').empty();
								console.log('Después de vaciar:', $('#modal_recogida .preview-container img').length);
					...
					success: function (res) {
						if (res.existe_presupuesto) {
							...
							$('#crearPresupuesto')
								.show()
								.data('id', res.cita.id)
								.data('id_tarea', res.cita.id_tarea);
			...
			$(document).on('click', '#crearPresupuesto, #editarPresupuesto', function() {
				var citaId = $(this).data('id');
				var tareaId = $(this).data('id_tarea');
				var presupuestoId = $(this).data('id_presupuesto') || null; // puede no existir al crear

				// Cambiar título del modal según acción
				var titulo = presupuestoId ? 'Editar Presupuesto' : 'Crear Presupuesto';
				$('#modalProductosLabel').text(titulo);

				// Vaciar tabla y mostrar mensaje de carga
				$('#tabla-productos-body').html('<tr><td colspan="4" class="text-center">Cargando productos...</td></tr>');

				// Abrir modal de selección de productos
				$('#modalSeleccionProductos').modal('show');

				// Petición AJAX para traer productos y estado de selección si es editar
				$.ajax({
					url: '<?= base_url('taller_citas/getProductosPresupuesto') ?>',
					method: 'POST',
					data: {
					...
					success: function(res) {
						if (res.success) {
							...
							res.productos.forEach(function(p) {
								filas += `
									<tr>
										<td><input type="checkbox" name="productos[]" value="${p.id}" ${p.seleccionado ? 'checked' : ''}></td>
										<td>${p.nombre}</td>
										<td>${p.descripcion}</td>
							...
							$('#tabla-productos-body').html(filas);
						} else {
							$('#tabla-productos-body').html('<tr><td colspan="4" class="text-danger text-center">No se pudieron cargar los productos.</td></tr>');
						}
					},
					error: function() {
						$('#tabla-productos-body').html('<tr><td colspan="4" class="text-danger text-center">Error en la petición.</td></tr>');
					}
				});
			});
