	...
	<?php foreach ($list_vehiculos as $vehiculo): ?>
		...
		>
			...
			<td class="col_sel">
				<div class="divImgModelo">
					<?php if($vehiculo->nombre_img) : ?>
						<img class="imgModelo" src="<?= base_url('assets_app/images/taller/'. $vehiculo->nombre_img ); ?>" 
							alt="<?php echo($vehiculo->nombre_img)?>" 
							value="<?php echo($vehiculo->nombre_img)?>" 
							data-url=<?= base_url('assets_app/images/taller/'. $vehiculo->nombre_img ); ?>
			...
			<td class="col_sel" id="columna_botones">
				<div id="div_botones">
					<?php if (isset($vehiculo->id) && $vehiculo->id != 0 && $acceso[3] == 1): ?>
						<!-- Botón de edición -->
