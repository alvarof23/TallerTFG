...
class Taller_vehiculos extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data["js"] = $this->load->view("taller_vehiculos/js_module/js_module", "", TRUE);
        $data['css'] = $this->load->view('taller_vehiculos/css_module/css_module', '', TRUE);
        ...
        $this->load->view('taller_vehiculos/list_vehiculos', $data);
    }
	...
	if (!$list_vehiculos) {
		...
		foreach ($list_vehiculos as $vehiculo) {
			...
			$cadena .= ' data-imagen="' . $vehiculo->nombre_img . '"';
			...
			$cadena .= '<td class="col_sel" id="columna_botones">';
			if (isset($vehiculo->id) && $vehiculo->id != 0) {
				...
				$cadena .= anchor("vehiculos/delete/" . $vehiculo->id, "<span class='fa fa-trash'></span>", 
							"class='text-danger delete' id='boton_vehiculos' data-placement='top' data-toggle='tooltip' data-original-title='Borrar vehículo'");
	...
	public function configuracion_calendario(){

		$tarea_id = $this->input->post('tarea_id');

		$configuraciones = $this->Taller_vehiculos_model->getHorasDisponibles($tarea_id);

		$todasCitas = $this->Taller_vehiculos_model->get_list_citas();

		foreach ($todasCitas as $evento) {

			var_dump($evento->hora); // Muestra el tipo de dato y el valor de 'hora'
			var_dump($evento->tiempo_estimado);	
			
			$eventos[] = [
				'id' => $evento->id,
				'title' => $evento->descripcion,
				'start' => $evento->hora,
				'end' => $evento->hora + $evento->tiempo_estimado,
				'url' => site_url('tus_rutas/editar/' . $evento->id),
